    ...
    res.json({
      ...
      data: {
        ...
        quantities: {
          ...
          ebmg:   String(sum("qtyEbms")),
          fo:     String(sum("qtyFo")),
          ldo:    String(sum("qtyLdo")),
          hsd:    String(sum("qtyHsd")),
          bhsd:   String(sum("qtyBioHsd")),
          xg:     String(sum("qtyXg")),
          sko:    String(sum("qtySko")),
        },
      },
    });
  }),
...
gateEntryRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const headers = [
      ...
      "EBMG (KL)", "XP95 (KL)", "FO (KL)", "LDO (KL)", "HSD (KL)", "B-HSD (KL)", "XG (KL)", "SKO (KL)", "Lock No",
    ...
    const rows = items.map((entry) => [
      ...
      entry.qtyMs ?? "", entry.qtyXpms ?? "", entry.qtyFo ?? "", entry.qtyLdo ?? "",
      entry.qtyHsd ?? "", entry.qtyBioHsd ?? "", entry.qtyXg ?? "", entry.qtySko ?? "",
      entry.lockNumber ?? "",
      ...
      (entry.invoiceNumber?.startsWith("MANUAL-") || entry.invoiceProductsRaw === "MANUAL") ? `${entry.invoiceNumber?.replace(/^MANUAL-/, "")} (Manual)` : (entry.invoiceNumber ?? ""), entry.invoiceDate?.toISOString().slice(0, 10) ?? "",
    ...
    const totalsRow = [
      "TOTALS", "", "", "", "", "", "", "",
      totalMs.toFixed(3), totalXpms.toFixed(3), totalFo.toFixed(3), totalLdo.toFixed(3),
      totalHsd.toFixed(3), totalBioHsd.toFixed(3), totalXg.toFixed(3), totalSko.toFixed(3), "",
      "", "", "", "", "", "", "",
      "", "", "",
      "", "", "", "", "", "",
    ];
...
gateEntryRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    sheet.columns = [
      ...
      { header: "EBMG (KL)", key: "ms", width: 10 },
      { header: "XP95 (KL)", key: "xp95", width: 10 },
      { header: "FO (KL)", key: "fo", width: 10 },
      { header: "LDO (KL)", key: "ldo", width: 10 },
      { header: "HSD (KL)", key: "hsd", width: 10 },
      { header: "B-HSD (KL)", key: "bioHsd", width: 12 },
      { header: "XG (KL)", key: "xg", width: 10 },
      { header: "SKO (KL)", key: "sko", width: 10 },
    ...
    items.forEach((entry) => {
      ...
      sheet.addRow({
        ...
        invoiceNo: (entry.invoiceNumber?.startsWith("MANUAL-") || entry.invoiceProductsRaw === "MANUAL") ? `${entry.invoiceNumber?.replace(/^MANUAL-/, "")} (Manual)` : (entry.invoiceNumber || ""),
