...
export default function EntryDetailPage() {
  ...
  return <div>
    <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><Link href="/entries" className="mb-3 inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-iocl-orange"><ArrowLeft className="h-4 w-4" />Back to records</Link><div className="flex flex-wrap items-center gap-3"><h1 className="text-2xl font-black text-iocl-navy sm:text-3xl">{entry.displaySerial}</h1><Badge tone={entry.status === "IN" ? "green" : entry.status === "OUT" ? "red" : "slate"}>{entry.status}</Badge><Badge tone={entry.ttNumberMatch ? "green" : "red"}>{entry.ttNumberMatch ? "TT Matched" : "TT Mismatch"}</Badge></div><p className="mt-2 text-sm text-slate-500">Created by {entry.createdBy.name} ({entry.createdBy.employeeCode})</p></div><div className="flex flex-wrap gap-2">{editing ? <><Button variant="secondary" onClick={() => { setDraft(toDraft(entry)); setQuantities(toQuantities(entry)); setEditing(false); }} icon={<X className="h-4 w-4" />}>Cancel</Button><Button loading={saving} onClick={() => void saveChanges()} icon={<Save className="h-4 w-4" />}>Save All</Button></> : <><Button variant="secondary" onClick={() => window.print()} icon={<Printer className="h-4 w-4" />}>Print</Button>{canEditOperational ? <Button onClick={() => setEditing(true)} icon={<Edit3 className="h-4 w-4" />}>{entry.status === "IN" ? "Edit Open IN" : user?.role === "ADMIN" ? "Admin Correct Record" : "Correct Record"}</Button> : null}</>}</div></div>
